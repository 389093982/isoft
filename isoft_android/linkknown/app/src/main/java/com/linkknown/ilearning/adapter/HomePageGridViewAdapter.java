...
public class HomePageGridViewAdapter extends BaseAdapter {
    ...
    private List<Catalog> listData;
    private LayoutInflater inflater;
    private Context context;
    private int mIndex;//页数下标，表示第几页，从0开始
    private int mPagerSize;//每页显示的最大数量

    public HomePageGridViewAdapter(Context context, List<Catalog> listData, int mIndex, int mPagerSize) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        Catalog catalog = listData.get(pos);
        holder.proName.setText(catalog.getCatalogName());
        Glide.with(this.context)
                .load(catalog.getCatalogIcon())
                .into(holder.imgUrl);
        return convertView;
    }

    ...
    <ISimpleLeftRightRow>
      <span slot="left">
        <Button type="success" size="small"
                @click="$router.push({ path: '/background/elementEdit', query: {placement_name:$route.query.placement_name}})">新增页面元素</Button>
      </span>
...
<script>
  import {CopyElement, FilterElements, ImportElement, UpdateElementStatus} from "../../../api"
  ...
  export default {
                ...
                h(MultiClickButton, {
                  ...
                  on: {
                    handleClick: async function (index, bindData) {
                      if (bindData == 2) {   // 编辑模式
                        _this.$router.push({
                          path: '/background/elementEdit',
                          query: {
                            id: _this.elements[params.index].id,
                            placement_name: _this.$route.query.placement_name
                          }
                        });
    ...
    methods: {
      ...
      refreshElementList: async function () {
        const result = await FilterElements({
          offset: this.offset,
          current_page: this.current_page,
          placement_name: this.$route.query.placement_name,
          search: this.search
        });
        if (result.status == "SUCCESS") {
